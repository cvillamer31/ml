...
def add_location_orig(user_id, user_location, user_date, user_time):
...
def getLogs2(id, date):
    try:
        ...
        db = get_db_connection()
        cursor = db.cursor(dictionary=True)
        query = 'SELECT id, date, date_out, in_time, out_time FROM attendances WHERE date BETWEEN %s AND %s AND worker_id = %s ORDER BY date, in_time DESC'
...
def add_location4(user_id, user_location, user_date, user_time, typeTerminal):
...
def get_attendance4():
    ...
    if request.method == 'POST':
        try:
            ...
            user_date = data['user_date']
            user_time = data['user_time']
            all_location = add_location4(user_id, user_location, user_date, user_time, "PWF");
            return all_location
        except Exception as e:
            return jsonify({'error': str(e)})
