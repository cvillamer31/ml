...
def gabor_filter(im, orient, freq, kx=0.65, ky=0.65):
    ...
    reffilter = np.exp(-(((x ** 2) / (sigma_x * sigma_x) + (y ** 2) / (sigma_y * sigma_y)))) * \
    ...
    finalind = np.where(
        (valid_row > block_size) & (valid_row < rows - block_size) &
    ...
    for k in range(np.shape(finalind)[1]):
        ...
        return_img[r][c] = np.sum(img_block * gabor_filter[orientindex[r // 16][c // 16] - 1])
